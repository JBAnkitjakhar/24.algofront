...
//     // Primary SuperAdmin protection - check by email since that's how backend identifies them
//     const PRIMARY_SUPER_ADMIN_EMAIL = "ankitjakharabc@gmail.com";
//     if (targetUser.email === PRIMARY_SUPER_ADMIN_EMAIL) return false;
    
//     // SuperAdmins cannot change other SuperAdmins (except Primary)
//     if (targetUser.role === UserRole.SUPERADMIN) return false;
...
//     // Only USER and ADMIN roles can be assigned (SuperAdmin can only be created by Primary)
//     return [UserRole.USER, UserRole.ADMIN];
//   },
...
//   // Check if user is Primary SuperAdmin
...
// src/lib/utils/auth.ts - UPDATED: Fixed Primary SuperAdmin validation for role assignment
