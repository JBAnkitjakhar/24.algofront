// src/components/admin/QuestionModals.tsx - Question management modals
...
function CodeSnippetsManager({ codeSnippets, onChange }: CodeSnippetsManagerProps) {
  const addCodeSnippet = () => {
    const newSnippet: CodeSnippet = {
      language: 'javascript',
      code: PROGRAMMING_LANGUAGES[0].template,
  ...
  const handleLanguageChange = (index: number, language: string) => {
    ...
    updateCodeSnippet(index, 'language', language);
    updateCodeSnippet(index, 'code', template);
  };
  ...
  return (
    <div className="space-y-4">
      {codeSnippets.map((snippet, index) => (
        <div key={index} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
          <div className="flex justify-between items-center mb-3">
            <h4 className="text-sm font-medium text-gray-700">Code Template {index + 1}</h4>
            <button
              type="button"
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Code Template</label>
            <textarea
              value={snippet.code}
      ...
      <div className="text-xs text-gray-500">
        Code templates help users get started with their solutions. Maximum 5 templates per question.
      </div>
    </div>
  );
}
...
export function CreateQuestionModal({ isOpen, onClose }: CreateQuestionModalProps) {
  ...
  const validateForm = (): string[] => {
    ...
    if (!formData.title.trim()) {
      ...
      if (formData.title.trim().length < QUESTION_VALIDATION.TITLE_MIN_LENGTH) {
        errors.push(`Title must be at least ${QUESTION_VALIDATION.TITLE_MIN_LENGTH} characters long`);
      }
      if (formData.title.trim().length > QUESTION_VALIDATION.TITLE_MAX_LENGTH) {
        errors.push(`Title must be less than ${QUESTION_VALIDATION.TITLE_MAX_LENGTH} characters`);
      }
    }
    ...
    if (!formData.statement.trim()) {
      ...
      if (formData.statement.trim().length < QUESTION_VALIDATION.STATEMENT_MIN_LENGTH) {
        errors.push(`Statement must be at least ${QUESTION_VALIDATION.STATEMENT_MIN_LENGTH} characters long`);
      }
      if (formData.statement.trim().length > QUESTION_VALIDATION.STATEMENT_MAX_LENGTH) {
        errors.push(`Statement must be less than ${QUESTION_VALIDATION.STATEMENT_MAX_LENGTH} characters`);
      }
    }
  ...
  const updateFormData = <T extends keyof CreateQuestionRequest>(field: T, value: CreateQuestionRequest[T]) => {
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-xs text-gray-500">
                      {formData.title.length}/{QUESTION_VALIDATION.TITLE_MAX_LENGTH} characters
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateFormData('level', e.target.value as QuestionLevel)}
                        className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                      >
                  ...
                  <div>
                    ...
                    <RichTextEditor
                      ...
                      onChange={(value) => updateFormData('statement', value)}
                      placeholder="Write your question statement here. You can include examples, explanations, and embed images..."
...
export function EditQuestionModal({ isOpen, onClose, question }: EditQuestionModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const hasChanges = 
      ...
      JSON.stringify(formData.imageUrls) !== JSON.stringify(question.imageUrls) ||
      JSON.stringify(formData.codeSnippets) !== JSON.stringify(question.codeSnippets);
    ...
    updateQuestionMutation.mutate(
      { id: question.id, request: formData },
      {
        onSuccess: () => {
          onClose();
        }
  ...
  const updateFormData = <T extends keyof UpdateQuestionRequest>(field: T, value: UpdateQuestionRequest[T]) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  };
...
export function DeleteQuestionModal({ isOpen, onClose, question }: DeleteQuestionModalProps) {
                ...
                <div className="mt-3 text-center">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Are you sure you want to delete <span className="font-medium text-gray-900">{question.title}</span>?
                    </p>
                    ...
                    <div className="mt-3 p-2 bg-gray-50 rounded border">
                      ...
                      <div className="flex items-center text-sm text-gray-600 mt-1">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${QUESTION_LEVEL_COLORS[question.level].bg} ${QUESTION_LEVEL_COLORS[question.level].text}`}>
